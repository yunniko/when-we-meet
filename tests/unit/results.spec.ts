...
function row(
  ...
  return { slotDate: new Date(`${date}T00:00:00Z`), slotHour: hour, status, preferred };
}

describe("computeResults", () => {
  it("includes every slot in the grid, even ones nobody marked", () => {
    const results = computeResults(dates, hours, 2, []);
  ...
  it("counts CAN/CANNOT/preferred per slot across participants", () => {
    const rows = [
      row("2026-08-21", 9, "CAN", true),
      row("2026-08-21", 9, "CAN", false),
      row("2026-08-21", 9, "CANNOT"),
    ];
    const results = computeResults(dates, hours, 3, rows);
  ...
  it("marks a slot as full-group only when everyone marked CAN", () => {
    const rows = [row("2026-08-21", 9, "CAN"), row("2026-08-21", 9, "CAN")];
    const results = computeResults(dates, hours, 2, rows);
  ...
  it("never marks full-group with zero participants", () => {
    const results = computeResults(dates, hours, 0, []);
  ...
  it("ranks by canCount desc, then preferredCount desc, then chronologically", () => {
    const rows = [
      ...
      row("2026-08-21", 9, "CAN"),
      // 2026-08-21 10:00 -> 2 can, 1 preferred
      row("2026-08-21", 10, "CAN", true),
      row("2026-08-21", 10, "CAN"),
      // 2026-08-22 09:00 -> 2 can, 0 preferred
      row("2026-08-22", 9, "CAN"),
      row("2026-08-22", 9, "CAN"),
      // 2026-08-22 10:00 -> 2 can, 2 preferred
      row("2026-08-22", 10, "CAN", true),
      row("2026-08-22", 10, "CAN", true),
    ];
    const results = computeResults(dates, hours, 2, rows);
